...
namespace Dapr.Client
{
    ...
    internal class DaprClientGrpc : DaprClient
    {
        ...
        private async Task<ByteString> GetStateAsync(string storeName, string key,
        ...
        public override async Task<TValue> GetStateAsync<TValue>(
            ...
            CancellationToken cancellationToken = default)
        {
            //Retrieve the ByteString value from the state
            var responseByteString = await GetStateAsync(storeName, key, consistencyMode, metadata, cancellationToken);

            try
            {
                return TypeConverters.FromJsonByteString<TValue>(responseByteString, this.JsonSerializerOptions);
            }
            catch (JsonException ex)
            {
        ...
        /// <inheritdoc/>
        public override async Task<ConditionalValue<TValue>> TryGetStateAsync<TValue>(string storeName, string key,
            ConsistencyMode? consistencyMode = default, IReadOnlyDictionary<string, string> metadata = default,
            CancellationToken cancellationToken = default)
        {
            //Retrieve the ByteString value from the state
            ...
            if (responseByteString.Length == 0)
            {
                return new ConditionalValue<TValue>(false, default);
            }

            var deserializedValue =
                TypeConverters.FromJsonByteString<TValue>(responseByteString, this.JsonSerializerOptions);
            return new ConditionalValue<TValue>(deserializedValue);
        }
        ...
        private async Task<(ByteString value, string etag)> GetStateAndETagAsync(string storeName, string key,
            ConsistencyMode? consistencyMode = default, IReadOnlyDictionary<string, string> metadata = default,
            CancellationToken cancellationToken = default)
        {
            ArgumentVerifier.ThrowIfNullOrEmpty(storeName, nameof(storeName));
        ...
        public override async Task<(TValue value, string etag)> GetStateAndETagAsync<TValue>(
            ...
            CancellationToken cancellationToken = default)
        {
            //Retrieve the ByteString result and its etag from the state
            var (byteStringResult, etag) =
                await GetStateAndETagAsync(storeName, key, consistencyMode, metadata, cancellationToken);
            
            try
            {
                return (TypeConverters.FromJsonByteString<TValue>(byteStringResult, this.JsonSerializerOptions),
                    etag);
            }
            catch (JsonException ex)
            {
        ...
        /// </returns>
        public override async Task<(ConditionalValue<TValue> value, string etag)> TryGetStateAndETagAsync<TValue>(
            string storeName, string key, ConsistencyMode? consistencyMode = default,
            IReadOnlyDictionary<string, string> metadata = default, CancellationToken cancellationToken = default)
        {
            //Retrieve the ByteString result and its etag from the state
            var (byteStringResult, etag) =
                await GetStateAndETagAsync(storeName, key, consistencyMode, metadata, cancellationToken);

            if (byteStringResult.Length == 0)
            {
                return (new ConditionalValue<TValue>(false, default), etag);
            }

            try
            {
                var deserializedResult =
                    TypeConverters.FromJsonByteString<TValue>(byteStringResult, this.JsonSerializerOptions);
                return (new ConditionalValue<TValue>(deserializedResult), etag);
            }
            catch (JsonException ex)
            {
                throw new DaprException(
