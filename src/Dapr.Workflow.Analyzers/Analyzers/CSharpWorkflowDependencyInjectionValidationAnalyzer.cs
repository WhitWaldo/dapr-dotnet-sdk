...
public sealed class CSharpWorkflowDependencyInjectionValidationAnalyzer : DiagnosticAnalyzer
{
	...
	// private static void AnalyzeSyntaxNode(SyntaxNodeAnalysisContext context, ISet<INamedTypeSymbol> registeredTypes,
	// 	INamedTypeSymbol workflowType)
	// {
	// 	//Get the syntax node and the semantic model
 ...
 //        }
 //
	// 	//Build a hash set representing all the DI-registered workflow types
	// 	registeredTypes.UnionWith(
	// 		TypeHelpers.IdentifyWorkflowDependencyInjectionTypeRegistrations(workflowType, node, model));
 //
	// 	//Get all the types in the compilation
	// 	var allTypes = TypeHelpers.GetAllTypes(context.Compilation.Assembly.GlobalNamespace);
 //
	// 	//Iterate through the types and compare to those identified in the dependency injection registration
	// 	//to report on those not found
	// 	foreach (var type in allTypes)
