...
public sealed class VersioningIntegrationTests
{
    ...
    public async Task ShouldResumeInFlightWorkflowWithOriginalVersionAndUseLatestForNew()
    {
        ...
        await using (var appV1 = await StartVersionedAppAsync(
                         environment,
                         options,
                         componentsDirV1,
                         selectorFactory: _ => new MinVersionSelector()))
        {
        ...
        await using (var appV2 = await StartVersionedAppAsync(
                         environment,
                         options,
                         componentsDirV2,
                         selectorFactory: _ => new MaxVersionSelector()))
        {
            ...
            var latestResult = await client2.WaitForWorkflowCompletionAsync(latestInstanceId, cancellation: latestCts.Token);
    ...
    [Fact]
    public async Task ShouldMarkWorkflowStalledWhenVersionMissing()
    {
        var instanceId = Guid.NewGuid().ToString("N");
        ...
        await using (var appV2 = await StartNonVersionedAppAsync(
                         environment,
                         options,
                         componentsDirV2))
        {
            ...
            var stalledState = await WaitForStatusAsync(client2, instanceId, WorkflowRuntimeStatus.Stalled,
                TimeSpan.FromMinutes(1));
            Assert.Equal(WorkflowRuntimeStatus.Stalled, stalledState.RuntimeStatus);
        }
    }
        ...
        TimeSpan timeout)
    {
        ...
        while (DateTime.UtcNow < stopAt)
        {
            ...
            if (state is not null && state.RuntimeStatus == status)
            {
                return state;
            }
        ...
        Assert.Fail($"Timed out waiting for workflow status {status}. Last status: {lastStatus}. Exists: {exists}.");
        return state!;
    }
